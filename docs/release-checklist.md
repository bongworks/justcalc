...
- [ ] 광고 없는 상태로 사용성·검색·성능 기준선을 수집한다. AdSense 신청 및 자동광고 활성화는 별도 승인·작업이다.
- [ ] 자동광고를 활성화할 때는 승인된 `NEXT_PUBLIC_ADSENSE_CLIENT_ID`로 프로덕션 산출물을 만들고 AdSense 관리 화면에서 자동 배치·광고 형식을 검토한다. 실제 채워진 광고로 모바일 계산 완료율·CLS·광고 위치·동의 흐름을 재검증한다. ID가 없는 로컬 점검만으로 완료하지 않는다.
