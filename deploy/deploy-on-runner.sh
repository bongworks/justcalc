...
restore_previous_release() {
  ...
  switch_to_release "$previous_release"
  restart_service
  health_check
}

source_directory="${1:-${GITHUB_WORKSPACE:-}/out}"
deploy_root="${DEPLOY_ROOT:-$HOME/justcalc}"
release_name="${RELEASE_NAME:-manual-$(date +%Y%m%d%H%M%S)}"
service_port="${SERVICE_PORT:-34560}"
service_host="${SERVICE_HOST:-0.0.0.0}"
launchd_label="${LAUNCHD_LABEL:-com.bongworks.justcalc}"
launchd_domain="${LAUNCHD_DOMAIN:-gui/$(id -u)}"
python_bin="${PYTHON_BIN:-$(command -v python3 || true)}"
script_directory="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
template_path="$script_directory/com.bongworks.justcalc.plist.template"
...
switch_to_release "$new_release"
restart_service

if ! health_check; then
  echo "Release health check failed: $new_release" >&2
